...
public:
	...
	static constexpr const char* WS_JS = "(()=>{let t=new WebSocket(`ws://${location.host}:81/ws`),e=0,n=1e4,o=null,i=null;function r(t){let e=JSON.parse(t.data);console.log(e);let n=e.data;if(200===e.status){for(let o of n.update)c(o);for(let i of n.remove)f(i)}}function a(){console.log('Disconnected'),clearInterval(o),i=setInterval(()=>{console.log('Reconnecting...'),(t=new WebSocket(`ws://${location.host}:81/ws`)).onopen=s,t.onmessage=r,t.onclose=a,t.onerror=l},5e3)}function s(){clearInterval(i),console.log('Connected'),o=setInterval(d,n)}function l(t){console.log(t)}function u(t){return document.querySelector(`[app-id='${t}']`)}function f(t){let e=u(t);e&&e.remove()}function c(t){let e=u(t['app-id']);if(e){for(;e.attributes.length>0;)e.removeAttribute(e.attributes[0].name);for(let n in t.attributes)e.setAttribute(n,t.attributes[n]);if(t.innerText&&(e.innerText=t.innerText),t.tag&&t.tag!==e.tagName){let o=document.createElement(t.tag);for(let i in e.attributes)o.setAttribute(i,e.attributes[i]);for(o.innerText=e.innerText;e.firstChild;)o.appendChild(e.firstChild);e.replaceWith(o)}}}function d(){!(Date.now()-e<n)&&(e=Date.now(),t.send(JSON.stringify({event:'ping'})))}function g(n,o){u(n).addEventListener(o,i=>{e=Date.now(),t.send(JSON.stringify({event:o,'app-id':n,data:i.target.value}))})}t.onopen=s,t.onmessage=r,t.onclose=a,t.onerror=l,window.app={addEvent:g}})();";
